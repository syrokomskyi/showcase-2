...
describe('Integration Tests', () => {
  describe('Full Article Processing', () => {
    ...
    it('should parse titles with emojis correctly', () => {
      ...
      expect(emojiTitle.getContent()).toBe('🍂 Warum gerade jetzt die richtige Zeit ist');
    });
    ...
    it('should parse marked lists correctly', () => {
      const markedLists = structures.filter(s => s.getType() === 'marked-list');
    ...
    it('should maintain correct order of structures', () => {
      ...
      const firstGuestIndex = structures.findIndex(s => s.getType() === 'guest');
      expect(firstGuestIndex).toBeGreaterThan(0);
      
      // Проверяем, что заголовки идут в логическом порядке
      const titleIndices = structures
        .map((s, i) => ({ type: s.getType(), index: i, content: s.getContent?.() || '' }))
        .filter(item => item.type.startsWith('title'));
      
      expect(titleIndices.length).toBeGreaterThan(5);
      
      // Проверяем иерархию заголовков
      const mainTitle = titleIndices.find(t => t.content.includes('🍂 Warum gerade jetzt'));
      const subTitle = titleIndices.find(t => t.content.includes('Der Dosenöffner'));
    ...
    it('should handle complex paragraph content', () => {
      ...
      const boldParagraph = paragraphs.find(p => p.content.includes('**Kleiner Tipp von mir:**'));
      expect(boldParagraph).toBeDefined();
      expect(boldParagraph.content).toContain('Bevor Sie neue Küchenhelfer kaufen');
