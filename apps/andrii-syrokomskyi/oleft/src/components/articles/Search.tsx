  ...
  const placeholderSuggestion = useMemo(() => {
    ...
    for (let i = 0; i < filteredArticles.length; i++) {
      ...
      if (
        ...
        suggestions.push(article.title.toLowerCase());
      }
    }
    ...
    <div className={className}>
      ...
      >
        ...
        <div className="relative">
          ...
          />
          {filteredArticles.length > 0 && showResults && searchValue && (
          ...
          return (
            <div
              className={classNames(
                ...
                {
                  hidden: !searchValue || filteredArticles.length === 0,
                },
                `fade-${state}`,
              )}
            >
              {filteredArticles.length > 0 ? (
                filteredArticles.map((article, index) => (
                  <a
                    key={index}
                    href={`/${getCountryName(country)}/${getLanguageName(language)}/articles/${article.slug}`}
                ...
                <div className="flex bg-white px-4 py-3 text-sm leading-none font-medium tracking-[-0.41px] text-appGray-500">
                  There are no articles for that search term
                </div>
              )}
            </div>
          );
